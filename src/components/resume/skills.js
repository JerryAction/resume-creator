  ...
  <span className="ml-2 text-gray-800">{category.name}</span>
              ...
              <div className="space-y-4">
                {category.skills.map((skill, skillIndex) => (
                  <div key={`${category.name}-${skill.name}-${skillIndex}`} className="group p-3 rounded-lg hover:bg-gray-50 transition-colors">
...
<div className="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
  ...
  background: index === 0 ? 'linear-gradient(to right, #8b5cf6, #a78bfa)' : 
             index === 1 ? 'linear-gradient(to right, #10b981, #34d399)' : 
             index === 2 ? 'linear-gradient(to right, #3b82f6, #60a5fa)' : 
             index === 3 ? 'linear-gradient(to right, #f59e0b, #fbbf24)' : 
